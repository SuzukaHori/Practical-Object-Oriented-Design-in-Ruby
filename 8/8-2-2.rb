...
# Arrayのサブクラスとして定義してsizeを生やす
# Arrayの全ての振る舞いを継承してしまう
# class Parts < Array
#   attr_reader :parts
...
# Enumurableをincludeしたもの
require 'forwardable'

class Parts
  extend Forwardable
  def_delegators :@parts, :size, :each # これがないとmountain_bike.parts.parts.sizeって書かなきゃいけない
  include Enumerable
  attr_reader :parts
...
front_shock =
  Part.new(name: 'front_shock', description: 'Manitou', needs_spare: false)
...
p mountain_bike.spares.size
p mountain_bike.parts.size
p mountain_bike.parts.count
# p mountain_bike.parts.each {|part| p part}
# p mountain_bike.parts.parts.size
